(async()=>{
    ...
    // blockly code
    var status_step, milliseconds, status2, status_type, temp, weeks, days, hours, minutes, seconds;
    ...
    s4d.client.on('ready', async () => {
      s4d.client.user.setPresence({status: "online",activities:[{name:'v.0.0..3 ALPHA',type:"COMPETING"}]});
    ...
    s4d.client.on('interactionCreate', async (interaction) => {
      ...
      if ((interaction.commandName) == 'sudo') {
        if (((interaction.member).id) == '844834622118035457') {
          ...
          if ((interaction.options.getString('command')) == 'status -c') {
          ...
          if ((interaction.options.getString('command')) == 'status -i') {
      ...
      if ((interaction.commandName) == 'warn') {
        if ((interaction.member).kickable) {
          ...
          try {
            ...
            await interaction.reply({ content: 'I can\'t find a channel named warnings :pensive: ', ephemeral: false, components: [] });
